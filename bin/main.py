...
def gey_yiyan():
    ...
    try:
        response = requests.get(yiyan_url)
        if response.status_code == 200:
            yiyan_data = response.json()
            return yiyan_data['hitokoto']
    except Exception as e:
        logging.error("获取一言API失败: {}".format(e))
        return "获取一言失败"

def display_image():
    global image
    global draw
    image = Image.new('1', (250, 122), 255)  # 创建图片对象
    draw = ImageDraw.Draw(image)  # 创建画笔对象
    font00 = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf', 20)
    font01 = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf', 18)
    font02 = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf', 16)
    font03 = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf', 14)
    ...
    font06 = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf', 8)
...
def main():
    logging.info("epd2in13V4 Demo")
    epd.init()  # 初始化
    epd.Clear(0xFF)  # 清屏操作1

    while True:
        display_image()  # 显示图片
        time.sleep(60)  # 更新间隔时间

    epd.Clear(0xFF)  # 清屏操作2
    epd.sleep()  # 使屏幕进入休眠状态
