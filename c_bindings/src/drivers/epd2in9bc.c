...
void EPD_2in9bc_Display(EPD *epd, const uint8_t *bk, const uint8_t *rd){if(!epd||!bk||!rd){return;}int sz=128*296/8;epd_send_command(epd,0x10);for(int i=0;i<sz;i++)epd_send_data(epd,bk[i]);epd_send_command(epd,0x13);for(int i=0;i<sz;i++)epd_send_data(epd,rd[i]);epd_send_command(epd,0x12);_b(epd);}
void EPD_2in9bc_Sleep(EPD *epd){if(!epd)return;epd_send_command(epd,0x02);_b(epd);epd_send_command(epd,0x07);epd_send_data(epd,0xA5);epd->delay_ms(2000);epd_teardown_default_hal(epd);}
