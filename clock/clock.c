 ...
 *   Ôø?display buffer (dx = 121-py, dy = px) in 122x250
 *
...
/* Global layout ‚Ä?set by layout_init, used by render functions */
Layout g_layout;
volatile int g_layout_refresh = 0;
volatile int g_layout_changed = 0;  /* layout refresh ‚Ü?skip clear/sleep */

/* Font sizes in pt ‚Ä?read from layout at runtime */
 ...
 *
 * Mapping: Python (px, py) Ôø?display (dx=121-py, dy=px)
 * ========================================================================= */
static void fb_to_display(void) {
    ...
    for (int py = 0; py < FB_HEIGHT; py++) {
        for (int px = 0; px < FB_WIDTH; px++) {
            ...
            if (pixel == 0) {
                /* Black pixel Ôø?clear bit in display buffer */
...
static void get_date_str(char *buf, size_t bufsize) {
    ...
    /* Gregorian date */
    char gregorian[64];
    strftime(gregorian, sizeof(gregorian), "%YÂπ?mÊú?dÊó?", tm_info);

    /* Weekday in Chinese */
    static const char *weekdays[] = {
        "ÊòüÊúüÊó?", "ÊòüÊúü‰∏Ä ", "ÊòüÊúü‰∫?", "ÊòüÊúü‰∏?", "ÊòüÊúüÂõ?", "ÊòüÊúü‰∫?", "ÊòüÊúüÂÖ?"
    };
...
static void get_ip_str(char *buf, size_t bufsize) {
    ...
    if (ip && strlen(ip) > 0) {
        ...
        snprintf(buf, bufsize, "Ëé∑ÂèñÂ§±Ë¥•");
        free(ip);
    }
}
...
/* Cached values for partial refresh comparison ‚Ä?extern for api.c */
...
static void draw_weather(void) {
    ...
    /* Prefix labels */
    ft_render_text(g_layout.w_label_x[0], g_layout.w_label_y[0], "Â§©Ê∞î:", FONT_SIZE_WEATHER, 0, 0, 127);
    ft_render_text(g_layout.w_label_x[1], g_layout.w_label_y[1], "Ê∏©Â∫¶:", FONT_SIZE_WEATHER, 0, 0, 127);
    ft_render_text(g_layout.w_label_x[2], g_layout.w_label_y[2], "ÊπøÂ∫¶:", FONT_SIZE_WEATHER, 0, 0, 127);
    ft_render_text(g_layout.w_label_x[3], g_layout.w_label_y[3], "ÂüéÂ∏Ç:", FONT_SIZE_WEATHER, 0, 0, 127);
...
static void partial_refresh(EPD *epd) {
    ...
    while (1) {
        ...
        if (g_layout_refresh) {
            ...
            printf("Layout changed ‚Ä?full refresh\n");
...
int main(void) {
    ...
    /* Load layout first ‚Ä?ft_init needs font_cn/font_time from it */
    ...
    while (1) {
        ...
        /* Layout change from API ‚Ä?skip clear/sleep, just redo basic_refresh */
        if (g_layout_changed) {
            g_layout_changed = 0;
            ft_reload();  /* reload fonts if font_cn/font_time changed */
            EPD_2in13_V4_Sleep(&epd);
            printf("Layout applied ‚Ä?immediate refresh\n");
            continue;
        }
